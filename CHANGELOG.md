...
### Added
- **reCAPTCHA V2 Badge Customization**: Added `badge` input support to the reCAPTCHA v2 component, supporting options `'bottomright' | 'bottomleft' | 'inline'`.
- **Playground Integration**: Added `badge` customization controls to the V2 Checkbox playground demo.

### Fixed
- **Testing Type Safety**: Fixed a TypeScript compiler error (`Property 'mockReturnValue' does not exist on type '() => Observable<boolean>'`) in `recaptcha-v2.component.spec.ts` by explicitly typing mock dependencies as `jest.Mock` and removing all usage of `any` types.
